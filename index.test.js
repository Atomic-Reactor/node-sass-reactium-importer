...
describe('Importer', function() {
    ...
    test('resolves to reactium_modules directory when first character is +', function() {
        mockFs.existsSync.mockReturnValueOnce(false).mockReturnValue(true);
        expect(importer('+my-module', '')).toEqual({
            file:
                __dirname + '/MOCK_PARENT_DIR/reactium_modules/my-module/index',
        });
    });

    test('does nothing when the first character isnt a ~ or +', function() {
    ...
    test('recursively resolve url until package has not been found', function() {
        ...
        for (var i = 0; i < 10; i++) {
            ...
            mockParentDirFinder = mockParentDirFinder.mockReturnValueOnce(
                'MOCK_PARENT_DIR' + i,
            );
        }
        ...
        mockParentDirFinder = mockParentDirFinder.mockReturnValueOnce(
            'MOCK_PARENT_DIR_final',
        );

        expect(importer('~my-module/test', '')).toEqual({
            file:
                __dirname +
                '/MOCK_PARENT_DIR_final/node_modules/my-module/test',
        });
    ...
    test('should resolve extensions', function() {
        ...
        expect(importer('~my-module/test.scss', '')).toEqual({
            file:
                __dirname + '/MOCK_PARENT_DIR/node_modules/my-module/test.scss',
        });
    });
    ...
    test('should support directory imports', function() {
        ...
        expect(importer('~my-module/test', '')).toEqual({
            file:
                __dirname +
                '/MOCK_PARENT_DIR/node_modules/my-module/test/index',
        });
    });
